...
class SemanticProcessor:
    ...
    def analyze_semantics(self, text):
        """
        تحلیل معنایی متن با استفاده از مدل معلم در صورت موجود بودن
        """
        if self.teacher:
            teacher_output = self.teacher.analyze_semantics(text)
            self.smart_model.learn_from_teacher(text, teacher_output)
            return teacher_output

        return self.smart_model.analyze_semantics(text)